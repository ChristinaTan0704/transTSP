...
#             context_node_projected=self.context_node_projected[key], # ! the average of all node's embeddings
...
#         else:  # TSP
#             assert problem.NAME == "tsp" or problem.NAME == "tspobs", "Unsupported problem: {}".format(problem.NAME)
...
#         """
#         :param input: (batch_size, task_num, node_dim) input node features or dictionary with multiple tensors
...
#         # ! _log_p : the probability; _log_p.size = (batch_size, task_num, task_num); the probabilty to be selected at each round (round: 0 ~ task_num)
#         # ! pi.size = (batch_size, task_num); stores the TSP solution for all samples in a batch
...
#         # Get log_p corresponding to selected actions # ! log_p.shape = [batch_size, task_num]
...
#         state = self.problem.make_state(input) # init a state

#         # Compute keys, values for the glimpse and keys for the logits once as they can be reused in every step
#         # ! fixed --- an AttentionModelFixed object that stores the K Q W extracted from the last layer's output embeddings
...
#             log_p, mask = self._get_log_p(fixed, state) # ! log_p.shape = [batch_size, 1, task_num]
...
#         """
#         :param input: (batch_size, task_num, node_dim) input node features
#         :return:
#         """
...
#     def _precompute(self, embeddings, num_steps=1): # embeddings.size = [batch_size, task_num, embed_dim]
#         # The fixed context projection of the graph embedding is calculated only once for efficiency
#         graph_embed = embeddings.mean(1) # graph_embed.size = [batch_size, embed_dim] 
#         # fixed context = (batch_size, 1, embed_dim) to make broadcastable with parallel timesteps
#         fixed_context = self.project_fixed_context(graph_embed)[:, None, :] # ! fixed_context.size = [batch_size, 1, embed_dim]

#         # The projection of the node embeddings for the attention is calculated once up front
#         # ! self.project_node_embeddings = nn.Linear(embedding_dim, 3 * embedding_dim, bias=False)
#         # ! glimpse_key_fixed.shape = [batch_size, 1, task_num, embed_dim], for each node we still have 128*1 embeddings
...
#         # ! fixed_attention_node_data is to reshape the K Q W (n_heads, batch_size, num_steps, task_num, head_dim)
...
#         # Compute logits (unnormalized log_p) # ! log_p.shape = [batch_size, 1, task_num]; glimpse.size = [batch_size, 1, embed_dim]
...
#         :param embeddings: (batch_size, task_num, embed_dim)
...
#         # Batch matrix multiplication to compute compatibilities (n_heads, batch_size, num_steps, task_num)
...
#         final_Q = glimpse
#         # Batch matrix multiplication to compute logits (batch_size, num_steps, task_num)
...
#             .permute(3, 0, 1, 2, 4)  # (n_heads, batch_size, num_steps, task_num, head_dim)
#         )

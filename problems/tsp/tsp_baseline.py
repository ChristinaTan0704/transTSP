...
from mapd.lib.transTSP.problems.vrp.vrp_baseline import get_lkh_executable
import torch
...
def solve_gurobi(directory, name, loc, disable_cache=False, timeout=None, gap=None):
    ...
    from mapd.lib.transTSP.problems.tsp.tsp_gurobi import solve_euclidian_tsp as solve_euclidian_tsp_gurobi
...
def nearest_neighbour(dataset, start='first'):
    ...
    batch_size, task_num, _ = dataset.size()

    total_dist = dataset.new(batch_size).zero_()

    if not isinstance(start, torch.Tensor):
        if start == 'random':
            start = dataset.new().long().new(batch_size).zero_().random_(0, task_num)
        elif start == 'first':
    ...
    current = start
    dist_to_startnode = torch.gather(dist, 2, current.view(-1, 1, 1).expand(batch_size, task_num, 1)).squeeze(2)
    tour = [current]

    for i in range(task_num - 1):
        # Mark out current node as option
        dist.scatter_(2, current.view(-1, 1, 1).expand(batch_size, task_num, 1), np.inf)
        nn_dist = torch.gather(dist, 1, current.view(-1, 1, 1).expand(batch_size, 1, task_num)).squeeze(1)
...
def solve_all_nn(dataset_path, eval_batch_size=1024, no_cuda=False, dataset_n=None, progress_bar_mininterval=0.1):
    ...
    from mapd.lib.transTSP.problems import TSP
    from mapd.lib.transTSP.utils import move_to
